...
export class ResetPasswordFrameComponent implements OnInit, OnDestroy {
  ...
  getError(el: any) {
    switch (el) {
      ...
      case 'password':
        ...
        } else if (this.newPasswordFormGroup.get('password')?.hasError('minlength')) {
          return 'Mật khẩu phải chứa tối thiểu 8 kí tự';
        }
        break;
      case 'confirm_password':
        if (this.newPasswordFormGroup.get('confirm_password')?.hasError('required')) {
          return 'Yêu cầu nhập lại mật khẩu xác thực';
        } else if (this.newPasswordFormGroup.get('confirm_password')?.hasError('minlength')) {
          return 'Mật khẩu xác thực phải chứa tối thiểu 8 kí tự';
        } else if (this.newPasswordFormGroup.get('confirm_password')?.hasError('password_not_match')) {
  ...
  onResend() {
    ...
    this.stopOtpTimer();
    this.removeOtp();
    this.usersService.resendCode(this.storageService.userInfo, {}).pipe(
      takeUntil(this.destroy$)
    ).subscribe(res => {
      this.storageService.userInfo = {
        ...this.storageService.userInfo,
  ...
  resetPassword(values: { password: string, confirm_password: string }) {
    ...
    this.usersService.resetPassword(this.storageService.userInfo, {}).pipe(
      takeUntil(this.destroy$)
    ).subscribe(res => {
  ...
  private createForms() {
    ...
    this.newPasswordFormGroup = this.formBuilder.group({
      ...
      confirm_password: ['', [Validators.required, Validators.minLength(this.PASSWORD_MIN_LENGTH)]],
    });
    this.createFormsEvents();
  }

  private createFormsEvents() {
    this.otpFormGroup.get('otp')?.valueChanges.pipe(
      takeUntil(this.destroy$)
    ).subscribe(value => {
      if (!!this.storageService.userInfo?.confirm_code
    ...
    this.newPasswordFormGroup.get('password')?.valueChanges.pipe(
      takeUntil(this.destroy$)
    ).subscribe(value => {
      const confirmPwField = this.newPasswordFormGroup.get('confirm_password');
      if (!!confirmPwField && value !== confirmPwField.value) {
        this.newPasswordFormGroup.get('confirm_password')?.setErrors({ 'password_not_match': true });
      } else {
        this.newPasswordFormGroup.get('confirm_password')?.setErrors(null);
      }
    });
    this.newPasswordFormGroup.get('confirm_password')?.valueChanges.pipe(
      takeUntil(this.destroy$)
    ).subscribe(value => {
